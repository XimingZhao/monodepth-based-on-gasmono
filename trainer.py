...
class Trainer:
    def __init__(self, options):
       ...
       # self.models["encoder"] = networks.ResnetEncoder(
        ...
        if self.opt.use_fc:
            self.models["fc"] = networks.fc()
            self.models["fc"].to(self.device)
            self.parameters_to_train += list(self.models["fc"].parameters())
    ...
    def generate_images_posegt(self, inputs, outputs):
        ...
        for frame_id in self.opt.frame_ids[1:]:
            ...
            if self.opt.use_fc:
                pose_OIS = []
                T_fc = self.models["fc"](T, pose_OIS)
            ...
            if self.opt.use_fc:
                pix_coords = self.project_3d[source_scale](
                    cam_points, inputs[("K", source_scale)], T_fc)
            else:
                pix_coords = self.project_3d[source_scale](
                    cam_points, inputs[("K", source_scale)], T)

            """
            T_OIS = inputs["pose_gt",frame_id].cuda().clone()
            cam_points = self.backproject_depth[source_scale](
                depth, inputs[("inv_K", source_scale)])
            pix_coords = self.project_3d[source_scale](
                cam_points, inputs[("K", source_scale)], T_OIS)
            """
